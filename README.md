...
- `sudo apt-get update`
- `sudo apt-get install -y build-essential` decrypt나 image resizing 설치할 때 에러가 발생하지 않는다.
- `sudo pat-get install curl`
...
백엔드 서버에서만 추가로 설치 (my SQL)

- `wget -c https://repo.mysql.com//mysql-apt-config_0.8.13-1_all.deb`
- `sudo dpkg -i mysql-apt-config_0.8.13-1_all.deb`
- `sudo apt-get update`
- `sudo apt-get install mysql-server` 8.0 버전 설치
- `sudo su` 해서 root 계정으로 전환
- `mysql_secure_installation` STRONG -> 비밀번호 지정 -> 전부 Y (나중에 비밀번호 변경해야한다.)
- `mysql -uroot -p`
